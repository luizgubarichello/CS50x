...
void add_pairs(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        for (int j = 0; j < candidate_count; j++)
        {
            ...
            {
                pairs[curr_pair].winner = i;
                pairs[curr_pair].loser = j;
                curr_pair++;
            }
        }
    }
    pair_count = curr_pair;
    return;
}
...
void sort_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        ...
        {
            int dif = preferences[pairs[j].winner][pairs[j].loser] - preferences[pairs[j].loser][pairs[j].winner];
            if (dif > temp)
            {
                temp = dif;
                tempj = j;
        ...
        pairs[i].winner = pairs[tempj].winner;
        pairs[i].loser = pairs[tempj].loser;
        pairs[tempj].winner = temp1;
        pairs[tempj].loser = temp2;
    }
    return;
}
